        ...
        match next {
            ...
            Work::Node(node) if dl::kind::ROOT == node.kind_id() => {
                ...
                let annotated_items = node
                    .children(&mut cursor)
                    .filter(|child_node| [dl::kind::ANNOTATED_ITEM].contains(&child_node.kind_id()))
                    .map(Work::Node);
            ...
            Work::Node(node) if dl::kind::ANNOTATED_ITEM == node.kind_id() => {
                ...
                let items = node
                    .children(&mut cursor)
                    .filter(|child_node| [dl::kind::ITEM].contains(&child_node.kind_id()))
                    .map(Work::Node);
                work.extend(items);
            },
            ...
            Work::Node(node) if dl::kind::FUNCTION == node.kind_id() => {
                ...
                let items = node
                    .children(&mut cursor)
                    .filter(|child_node| {
                        [dl::kind::FUNCTION_EXTERN, dl::kind::FUNCTION_NORMAL].contains(&child_node.kind_id())
                    })
                    .map(Work::Node);
                work.extend(items);
            },
            ...
            Work::Node(node) if dl::kind::REL == node.kind_id() => {
                ...
                let items = node
                    .children(&mut cursor)
                    .filter(|child_node| [dl::kind::REL_ARGS, dl::kind::REL_ELEM].contains(&child_node.kind_id()))
                    .map(Work::Node);
                work.extend(items);
            },
            ...
            Work::Node(node) if dl::kind::TYPEDEF == node.kind_id() => {
                ...
                let items = node
                    .children(&mut cursor)
                    .filter(|child_node| {
                        [dl::kind::TYPEDEF_EXTERN, dl::kind::TYPEDEF_NORMAL].contains(&child_node.kind_id())
                    })
                    .map(Work::Node);
                work.extend(items);
            },
